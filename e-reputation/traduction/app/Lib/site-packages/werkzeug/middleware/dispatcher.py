...
This example dispatches to an API stream under "/api", an admin stream
under "/admin", and an stream that serves frontend files for all other
requests::

    stream = DispatcherMiddleware(serve_frontend, {
        '/api': api_app,
        '/admin': admin_app,
    })
